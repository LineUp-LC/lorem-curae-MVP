...
> Trigger: Read this file when handling auth, env vars, API keys, Supabase, Stripe, user data, or deployment safety.
...
- NEVER fabricate product data, ingredient properties, or retailer info
- NEVER make medical diagnoses or guarantee skincare results
- NEVER copy text from external skincare databases (INCIdecoder, Paula's Choice, etc.)
- All ingredient content must be original
- Scientific claims must use conditional language ("may help", "commonly associated with")
...
- Personalization data cascade: Supabase → sessionState → localStorage → null
