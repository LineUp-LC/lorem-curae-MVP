...
> Trigger: Read this file when implementing features, making decisions, proposing changes, or handling session recovery.
...
- Modify Supabase schema or Stripe logic
- Break mobile layout
...
This repository uses two governance files:

| File | Scope |
|------|-------|
| `CLAUDE.md` (root) + `.claude/rules/` | Developer governance — architecture, code, workflow |
| `ai-governance/CLAUDE_PRODUCT.md` | User-facing AI — tone, safety, content rules |

**Runtime sync:** `src/lib/ai/systemPrompt.ts` contains the runtime-optimized version of `CLAUDE_PRODUCT.md`. Changes to product governance require updates to both files.
