...
import { getEffectiveConcerns, getEffectivePreferences, getEffectiveSkinType } from '../../../lib/utils/sessionState';
...
const SimilarProducts = ({ productId }: SimilarProductsProps) => {
  ...
  // Check if concern matches user's concerns (survey-aware via matching.ts)
  const isConcernMatching = (concern: string): boolean => {
    return matchesConcern(concern, userConcerns);
  };

  // Check if ingredient is beneficial for user's concerns
  const isIngredientMatching = (ingredient: string): boolean => {
    return matchesIngredient(ingredient, userConcerns);
  };
  ...
  // Rank similar products by relevance to user profile + current product
              ...
              >
                ...
                <div className="p-5">
                  ...
                  >
                    ...
                    <span className="text-sm text-gray-500 hover:text-primary transition-colors">({product.reviewCount})</span>
                  </Link>
